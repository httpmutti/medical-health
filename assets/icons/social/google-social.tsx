/* /assets/icons/ui/google-social.tsx */

import { ReactElement } from 'react';
import { SvgXml } from 'react-native-svg';

interface GoogleSocialSvgProps {
  primaryColor?: string;
  secondaryColor?: string;
  color?: string;
  size?: string | number;
}

const GoogleSocialSvg = ({ primaryColor, secondaryColor, color, size: defaultSize = 40 }: GoogleSocialSvgProps): ReactElement => {
  const resolvedPrimary = primaryColor ?? color ?? "#2260FF";
  const resolvedSecondary = secondaryColor ?? resolvedPrimary;

  return (
    <SvgXml
      width={defaultSize}
      height={defaultSize}
      xml={`<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M16.0978 3.38055C14.578 1.98052 12.6227 1.14612 10.5603 1.01747C8.49799 0.888817 6.45417 1.47376 4.77216 2.67405C3.09015 3.87434 1.8724 5.61687 1.32343 7.60897C0.774458 9.60108 0.927706 11.7214 1.75744 13.6139C2.58717 15.5063 4.04284 17.0556 5.87997 18.0016C7.7171 18.9475 9.82378 19.2325 11.8462 18.8086C13.8686 18.3847 15.6836 17.2778 16.9863 15.6738C18.289 14.0698 19 12.0664 19 10L10 10" stroke="#2260FF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg>



`}
    />
  );
};

export default GoogleSocialSvg;